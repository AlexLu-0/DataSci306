...
regions <- c("Africa", "World", "Asia", "Latin America and the Caribbean", "Northern America", "Europe")
combined_data <- data_estimates %>%
  filter(Year <= 2024) %>%
  bind_rows(data_medium_variant %>% filter(Year > 2024)) %>%
...
combined_data <- combined_data %>%
  filter(!is.na(Year) & !is.na(FertilityRate))

ggplot(combined_data, aes(x = Year, y = FertilityRate, color = Region)) +
  geom_line(size = 1) +
  scale_x_continuous(
    limits = c(1950, 2100),
...
combined_data <- data_estimates %>%
  ...
  select(`Region, subregion, country or area *`, Year, `Total Population, as of 1 July (thousands)`)
...
ggplot(combined_data, aes(x = Year, y = Population, color = Country)) +
  geom_line(size = 1) +
  scale_x_continuous(
    limits = c(1950, 2100),
    breaks = c(1950, 1980, 2000, 2020, 2020, 2040, 2060, 2080, 2100)
  ) +
  scale_y_continuous(
    limits = c(0, 1800000000),
    breaks = c(0, 200000000, 400000000, 600000000, 800000000, 1000000000, 1200000000, 1400000000, 1600000000),
    labels = c("0", "200 million", "400 million", "600 million", "800 million", "1 billion", "1.2 billion", "1.4 billion", "1.6 billion")
  ) +
  labs(
    title = "Population of China and India, 1950 to 2100",
    x = "",
    y = "",
    color = "Country"
  ) +
...
```{r}
countries <- c("United States of America", "Germany", "Democratic Republic of the Congo", "Japan", "India", "Nigeria")

age_data <- data_estimates %>%
  filter(Year <= 2023) %>%
  bind_rows(data_medium_variant %>% filter(Year > 2023)) %>%
  filter(`Region, subregion, country or area *` %in% countries) %>%
  select(`Region, subregion, country or area *`, Year, `Median Age, as of 1 July (years)`) %>%
  rename(
    Country = `Region, subregion, country or area *`,
